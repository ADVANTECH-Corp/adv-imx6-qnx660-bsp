...
disable_tx_on_empty( HW_CONTEXT_T *mx )
{
    ...
    mx->ssi->sisr |= SISR_TUE0; /* Clear transmitter underrun status */
    ...
    while(!(mx->ssi->sisr & SISR_TUE0) && i-- > 0);
        nanospin_ns(100);
    if (i <= 0)
        ado_error("%s: TXSR failed to empty, sisr = 0x%x", __FUNCTION__, mx->ssi->sisr);
...
#if defined (VARIANT_MX6X) || defined (VARIANT_BT) || defined (VARIANT_AIC3104)
            __attribute__((unused)) uint32_t tmp;   /* Use unused attribute to silence compiler warning */
            uint32_t cnt = 0;
...
static int open_log (HW_CONTEXT_T * mx, int chn_type)
{
    ...
    {
        snprintf(filename_buf, _POSIX_PATH_MAX, "/dev/shmem/playback_combine_%04d%02d%02d-%02d%02d%02dUTC.wav",
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/deva/ctrl/mx/mxssi_dll.c $ $Rev: 778903 $")
#endif
