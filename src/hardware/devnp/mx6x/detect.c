...
mx6q_detect(void *dll_hdl, struct _iopkt_self *iopkt, char *options)
{
    ...
    for(i = 0; ; i++) {
        mx6q_arg.idx = i;
        dev = NULL; /* No parent */
        if(dev_attach("fec", options, &mx6q_ca, &mx6q_arg, &single, &dev, NULL) != 0) {
            break;
        }
    }
    if(i == 0) {
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/mx6x/detect.c $ $Rev: 772270 $")
#endif
