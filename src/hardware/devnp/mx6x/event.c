...
mx6q_process_interrupt(void *arg, struct nw_work_thread *wtp)
{
    ...
    for (;;) {
        ...
        /* Read and clear interrupt cause bits, ignoring Rx and MII */
#ifdef MX6XSLX
        ievent = *(base + MX6Q_IEVENT) & ~(IEVENT_RXF2 | IEVENT_RXF1 |
                           IEVENT_RFINT | IEVENT_MII);
#else
        ievent = *(base + MX6Q_IEVENT) & ~(IEVENT_RFINT | IEVENT_MII);
#endif
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/mx6x/event.c $ $Rev: 772270 $")
#endif
