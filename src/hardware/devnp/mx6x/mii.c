...
int mx6_is_br_phy (mx6q_dev_t *mx6q) {
    uint32_t PhyId = 0, vendor= 0, model=0;
    ...
    if (is_br == -1) {
        ...
        if (PhyAddr == -1) {
            if (mx6q->cfg.verbose) {
                log(LOG_ERR, "%s(): PhyAddr = %d", __FUNCTION__, PhyAddr);
            }
        } else {
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/mx6x/mii.c $ $Rev: 772270 $")
#endif
